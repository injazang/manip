...
def train_epoch(model, loader, logger, optimizer, epoch, n_epochs, mixer, use_mix='mix', print_freq=1, mix_batch_prob=0, mix_spatial_prob=0 ):
    ...
    for batch_idx, inputs in enumerate(loader):
        ...
        if torch.cuda.is_available():
            ...
            target = target.repeat(1, 128 * 128).view(32, 128, 128)
        ...
        error.update(torch.ne(pred.argmax(dim=1), target.cpu().squeeze()).sum().item() / (batch_size * 128 * 128), batch_size)
...
def test_epoch(model, loader, logger, print_freq=1, is_test=True):
    ...
    with torch.no_grad():
        for batch_idx, inputs in enumerate(loader):
            if torch.cuda.is_available():
                im = inputs['im'].cuda()
                target = inputs['label'].cuda().view(-1, 5).float()
...
def test(model, test_set, logger, batch_size=32):
    ...
    test_loader = torch.utils.data.DataLoader(test_set, batch_sampler=test_sampler, pin_memory=(torch.cuda.is_available()))
...
if __name__ == '__main__':
    demo('mantra', gpu=0, training='train',n_epochs=200, batch_size=32, use_mix='mix', jpeg=False)
