...
import time
import torch
from datasets.ManipDataset_DCT import ConcatSampler, ManipDataset
...
def train(model, optimizer, train_csvs, val_set,test_set, logger, model_dir,
    ...
    val_loader = torch.utils.data.DataLoader(val_set, batch_sampler=val_sampler, pin_memory=(torch.cuda.is_available()), num_workers=8)
...
def demo(model, gpu, training='train',load=None,fine_tune=True, n_epochs=200, batch_size=32, use_mix='mix', jpeg=False,load_srnet=None, load_8 = None, load_4=None):
    ...
    # Model
    from models.SRNet_DCT_scale import SRNet as SRNet_DCT
    from models.SRNet import SRNet

    dct8 = SRNet_DCT(scale=8, load=True).cuda()
    dct4 = SRNet_DCT(scale=4, load=True).cuda()
    srnet = SRNet(load=True).cuda()

    if load is None:
        last_checkpoint_path = glob(os.path.join('trained_/'+load_4, '*.tar'))[-1]
        print(last_checkpoint_path)
        checkpoint = torch.load(last_checkpoint_path, map_location=f'cuda:{gpu}')
        dct4.load_state_dict(checkpoint['model_state_dict'], strict=False)
        for param in dct4.parameters():
            param.requires_grad=False

        last_checkpoint_path = glob(os.path.join('trained_/' + load_8, '*.tar'))[-1]
        print(last_checkpoint_path)
        checkpoint = torch.load(last_checkpoint_path, map_location=f'cuda:{gpu}')
        dct8.load_state_dict(checkpoint['model_state_dict'], strict=False)
        for param in dct8.parameters():
            param.requires_grad=False

        last_checkpoint_path = glob(os.path.join('trained_/' + load_srnet, '*.tar'))[-1]
        print(last_checkpoint_path)
        checkpoint = torch.load(last_checkpoint_path, map_location=f'cuda:{gpu}')
        srnet.load_state_dict(checkpoint['model_state_dict'], strict=False)
        for param in srnet.parameters():
            param.requires_grad=False

    from models.SRNet_pyramid import SRNet_pyramid
    model = SRNet_pyramid(srnet, dct4, dct8).cuda()
    ...
    if training=='train':
        ...
        train(model=model, optimizer=optimizer, train_csvs=csvs[0:5], val_set=val_set, test_set=test_set,
            ...
            n_epochs=n_epochs, batch_size=batch_size, jpeg=jpeg, fine_tune=fine_tune)
...
if __name__ == '__main__':
    demo('pyramid', gpu=0, training='train',n_epochs=100, batch_size=100, fine_tune=True, use_mix='mix', jpeg=True, load=None, load_srnet='srnet_JPEG_20-06-21_01-25', load_4='dct_4', load_8='dct_8')
