...
def train(model, optimizer, train_csvs, val_set, test_set, logger, model_dir, epoch, best_error, lr=0.0, wd=0.0,
          fine_tune=False,
...
def demo(model=None, gpu=None, training='train', load=None, num_labels=16, n_epochs=200, batch_size=32, use_mix='mix',
         datadir='',
    ...
    if training == 'train':
        # Train the model
        train(lr=lr, wd=wd, model=model, optimizer=optimizer, train_csvs=glob(f'{datadir}/*_jpg.txt'), val_set=val_set,
              test_set=test_set, num_labels=num_labels,
