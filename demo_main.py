...
def test_epoch(model, loader, logger, print_freq=1, is_test=True, confusion = False):
    ...
    with torch.no_grad():
        for batch_idx, inputs in enumerate(loader):
            ...
            error.update(torch.ne(pred, target).sum().item() / (batch_size),
                         batch_size)
            losses.update(loss.item(), batch_size)
            if confusion:
...
if __name__ == '__main__':
    demo('srnet', gpu=0, training='test', n_epochs=200, batch_size=100, use_mix='mix', num_labels=20,
         jpeg=True, load='srnet_JPEG__20_20-07-25_04-40')
